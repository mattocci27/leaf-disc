...
\tilde{\mu_i} & = \beta_0 + \beta_1 \mathrm{ln}(LD_{i}) + \beta_2 \mathrm{ln}(LA_{i}) + \beta_3 \mathrm{ln}(LT_{i}) (\#eq:mu)
\end{align}

where $LMA_{Wi}$, $LMA_{Di}$, $LD_{i}$, $LA_{i}$, $LT_{i}$ are the observed whole-leaf LMA, leaf disc LMA, leaf tissue density, leaf area and leaf thickness for tree *i*, respectively,
$\tilde{\mu_i}$ is the effect of leaf tissue density, leaf area and leaf thickness on the mean estimates for the whole-leaf LMA of tree *i*, and $\sigma_i^2$ is the variance of whole-leaf LMA for tree *i*, and $\beta$ are coefficients.
...
\tilde{\sigma_i} & = \gamma_0 + \gamma_1 \mathrm{ln}(LD_{i}) + \gamma_2 \mathrm{ln}(LA_{i}) + \gamma_3 \mathrm{ln}(LT_{i}) (\#eq:tilde-sigma)
\end{align}
...
Bivariate trait relationships between disc-based and whole-leaf log-transformed LMA were further summarized with variance explained (*R^2^*) based on Pearson correlations and with standardized major axis (SMA) regressions [@Warton2006] across all the samples and different leaf area and thickness groups.
...
and thin-leaved species (LT < `r val$LT_mid` mm).
...
Leaf tissue density, leaf area and leaf thickness influenced the relationship between the whole-leaf LMA and leaf disc LMA (Fig. 1).
Specifically, species with lower leaf tissue density (Fig. 1a, $\beta_1$) and larger leaves (Fig. 1a, $\beta_2$) had greater whole-leaf LMA than leaf disc LMA.
Species with lower leaf tissue density (Fig. 1b, $\gamma_1$) and larger larger leaves (Fig. 1b, $\gamma_2$) also had larger variance in the relationship between the whole-leaf LMA and leaf disc LMA.
Thinner-leaved species (Fig. 1b, $\gamma_3$) tended to have larger variance in the relationship (i.e., the 90% credible interval did not include zero).
...
Species with less dense, thicker and smaller leaves had stronger correlations between disc-based and whole-leaf estimates of LMA (Table 1, Fig. 1).
...
**Fig. 1.**
Standardized regression coefficients modelig
(a) the effects of leaf tissue density (LD; $\beta_1$),
leaf area (LA; $\beta_2$),
leaf thickness (LT; $\beta_3$)
on the mean estimates of whole-leaf LMA, and
(b) the effects of leaf tissue density ($\gamma_1$),
leaf area ($\gamma_2$), and
leaf thickness ($\gamma_3$)
on the estimated variance of whole-leaf LMA.
...
**Fig. 2.**
Relationships between species mean leaf mass per area (LMA) determined by using whole leaves and leaf discs for (a) nondense-leave species and (b) dense-leaved species.
Species groups are divided into eight categories on the basis of the medians of the leaf tissue density, leaf area and leaf thickness across all the species.
