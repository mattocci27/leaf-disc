...
Leaf mass per area (LMA), determined by lamina thickness and leaf tissue density (LD), describes how much biomass is invested into given photosynthetic leaf area, which is the key trait of the leaf economic spectrum [@Wright2004a; @Poorter2009; @Onoda2017].
...
Previous works that compared estimates of whole-leaf LMA and leaf disc LMA using tropical tree species showed good correlations between the two [*R^2^* = 0.92 for whole-leaves in @Kraft2008; *R^2^* = 0.92 for leaf laminas in @Onoda2011], but the comparisons between the two estimates in other biomes have rarely been examined.
...
The extent of ITV within and among communities for LMA can be similar to or greater than interspecific trait variation [e.g., @Messier2010; @Kichenin2013; @Fajarado2018].
...
We predicted that whole-leaf LMA would be greater than leaf disc LMA because whole-leaf LMA includes petioles and midribs that have greater dry mass per unit area than laminas [@Niinemets2007].
...
In total, our data set comprised `r val$all_sp` woody species and `r val$all_tree` individual that ranged from 8.1 to 24.7 °C in mean annual temperature (MAT) and 732.8 to 4477 mm in mean annual precipitation (MAP) [Table S1; @Eguchi2006; @Song2017; @Fei2018], which covers the wide climate ranges in the distribution of broad-leaved tree species.
...
A bivariate trait relationships between disc-based and whole-leaf log-transformed LMA were summarized with variance explained (*R^2^*) based on Pearson correlations and with standardized major axis (SMA) regressions [@Warton2006] across all the species.
...
All statistical analyses were conducted in R version 4.1.3 [@RCoreTeam2021].
...
Altough there were no stiatical differences in the SMA slope or elevation(SMA slope: `r val$sma_slope_cv_mean` [95%CI: `r val$sma_slope_cv_lwr`, `r val$sma_slope_cv_upr`], SMA intercept: `r val$sma_int_cv_mean` [95%CI: `r val$sma_int_cv_lwr`, `r val$sma_int_cv_upr`]), leaf disc LMA had greater CV than whole-leaf LMA (*t* = `r val$t_test_t`, *d.f.* = `r val$t_test_df`, *P* < 0.001).
