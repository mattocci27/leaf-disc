...
Thank you.
We now note that a key difference between leaf disc and whole-leaf LMA is that leaf disc LMA focuses on lamina rather than whole-leaf and usages depend on ecological contexts.
We also have mentioned the TRY database to explain LMA is the most often used leaf trait in TRY.
...
We now use a linear model that includes leaf area and leaf thickness as covariates but we suggest OLS models without covariates for the conversion of LMA because leaf thickness and leaf area affect not only the mean but also the variance in the relationship between the leaf disc and whole-leaf estimates, and that is why R2 values changed among the four groups (thin vs. think and small vs. large leaves) and it is not to practical to convert LMA using leaf area or leaf thickness.
We also present a practical guideline for conversion of LMA.
...
Thank you.
We have cited the recent review by Westerband et al. as well as other works to explain ITV, its importance and the extent of ITV for LMA.
...
Thank you.
We now introduce the LES in the first sentence so that the readers can understand the role of LMA in LES more clearly.
...
Thank you.
We now break up the paragraph into two sections: the first describing the different methods and their record numbers in TRY, and previous works that compared them, and the second describing the expected effects of leaf morphology on the relationship between whole-leaf and leaf disc LMA.
...
Thank you.
We have repeated our expectations here.
...
Thank you.
We have started the result section with the shift in elevation between whole-leaf and leaf disc LMA (i.e., whole-leaf LMA is greater than leaf disc LMA) .
...
We consider this as a limitation of our study and explain this in the discussion.

> L183-184 Which metric had higher ITV? That is a more relevant result than simply the R2 value.

Leaf disc LMA had higher ITV.
We have added the paired t-test for this in the results section.
...
Thank you.
The two panels share the x axis.
Units are in the strip.
...
Thank you.
We have mentioned several ecological studies that used leaf disc LMA in the introduction.

> The authors had better explain why they used the LMA values with leaf petioles mass.

Thank you.
We have added the records of TRY database that shows LMA including petioles are very common.
...
Thank you.
We have cited it in the context of "diminishing returns" in the discussion.
...
Thank you.
We have added MAT and MAP in the main text to support that our dataset covers the wide climate ranges in the distribution of broad-leaved tree species.
...
We now more clearly explain that thick veins are secondary order veins.
...
Thank you.
We have added confidence intervals using bootstraps in the result section.
