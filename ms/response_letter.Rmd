...
This allows to quantify when the whole-leaf LMA to leaf disc LMA ratio become large and in which direction the relationship is changing.

2. We also removed the analysis that grouped species into four categories based on medians of leaf area and leaf thickness because it is somewhat arbitral as pointed by the reviewer.
Instead, the above Bayesian model can was used to test our hypotheses.

3. We have removed the individual-based analysis from the main text because variance among species are not independent in the individual-level data, and thus SMA or OLS regressions are not appropriate.
